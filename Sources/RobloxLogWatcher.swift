...
final class RobloxLogWatcher: ObservableObject {
    ...
    private func resolveGameName(placeId: String) {
        // 🛡️ Sentinel: Validate placeId to prevent SSRF / injection (only digits allowed)
    ...
    private func resolveRegion(ip: String) {
        // 🛡️ Sentinel: Validate IP to prevent SSRF / injection (IPv4/IPv6 allowed)
    ...
    private nonisolated static func measurePing(ip: String?) -> Int? {
        ...
        // 🛡️ Sentinel: Validate IP to prevent command injection via process arguments
